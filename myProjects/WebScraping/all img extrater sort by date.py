...
# this program downloads all files regardless of same name, sort by date after downloading is complete to get it in sequence
...
# qutored section gives the list of all links to comics present on first few and the last page
...
for i in range(1):
    ...
    for l in range(len(imglinks)):
        ...
        if imglinks == []:
         ...
         res.raise_for_status()
         imgFilePath = os.path.join(Foldername,str(l)+'s'+'.jpg')
         print(imgFilePath)
...
for j in range(len(nextlinks)):
    ...
    for k in range(len(imglinks)):
        ...
        if imglinks == []:
            ...
            res = requests.get(imgurl)
            res.raise_for_status()
            imgFilePath = os.path.join(Foldername, (str(j)+str(k) +'.jpg'))
            print(imgFilePath)
            imgFile = open(imgFilePath,'wb')
            #print('newfile')
            for chunk in res.iter_content(100000):
